...
const pocketHint = (type: TransactionType, hasSrc: boolean, hasDest: boolean): string | null => {
...
export const TransactionFormAccountSection = ({ form, accounts, pots, pockets }: Props) => {
	const filteredPots = pots.filter((p) => p.accountId === form.accountId);
	const selectedPot = filteredPots.find((p) => p.id === form.potId);
	const isPotLow =
		selectedPot &&
		(selectedPot.amountLeft <= 0 || selectedPot.amountLeft / selectedPot.limitAmount <= 0.1);
		...
		<div className="grid grid-cols-1 gap-4">
					...
					{isPotLow && (
						<div className="mt-2 flex items-center gap-2 text-[10px] text-amber-400 bg-amber-500/10 p-2 rounded-lg border border-amber-500/20">
							...
							<span>
								...
								{selectedPot?.amountLeft && selectedPot.amountLeft <= 0
									? "Exceeded"
									: "Less than 10% left"}
								)
							</span>
			...
			{showPocketSection && (
				<div className="animate-fadeIn space-y-4">
					...
					{hint && <p className="mt-1 text-[9px] text-indigo-400 font-medium italic">{hint}</p>}
				</div>
			)}
