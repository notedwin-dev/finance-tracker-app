...
const FiltersHeader: React.FC<{ onClose?: () => void }> = ({ onClose }) => (
...
const AccountsSection: React.FC<{
	...
	clearAccounts: () => void;
}> = ({ accountIds, accounts, toggleAccount, clearAccounts }) => (
...
const buildShowResultsLabel = (
	hasActiveFilters: boolean,
	accountIdsCount: number,
	hasDateRange: boolean,
): string => {
	if (!hasActiveFilters) return "Show All Transactions";
	const accountsLabel =
		accountIdsCount > 0
			? `${accountIdsCount} account${accountIdsCount > 1 ? "s" : ""}`
			: "All";
	return `Show Results (${accountsLabel} · ${hasDateRange ? "Dates" : "All time"})`;
};

const ShowResultsButton: React.FC<{
	hasActiveFilters: boolean;
	accountIdsCount: number;
	hasDateRange: boolean;
	onClick: () => void;
}> = ({ hasActiveFilters, accountIdsCount, hasDateRange, onClick }) => (
	<button
		onClick={onClick}
	...
	>
		{buildShowResultsLabel(hasActiveFilters, accountIdsCount, hasDateRange)}
	</button>
);
...
const FiltersPanel: React.FC<Props> = ({
	...
	const filterBody = (
		<div className="space-y-6">
			<FiltersHeader onClose={onClose} />
			{hasActiveFilters && <ClearAllButton onClick={onClear} />}
			<AccountsSection
				accountIds={accountIds}
				accounts={accounts}
				toggleAccount={toggleAccount}
				clearAccounts={() => onAccountIdsChange([])}
			/>
			<DateRangeSection
				startDate={startDate}
				endDate={endDate}
